...
void CallCRHisto_creation (TSelectionAnalyzer* Analyzer, int variable_bitcode, int CR_bitcode, TString control_region, TString variable_name) {
  ...
  for (unsigned int is=0; is<=nSA; is++){ //loop over samples
    ...
    if(useRatioBinning ){ 
    	Analyzer->getCRHisto(presel_file, variable_bitcode|CR_bitcode|RAT0 , path_sim+s_CR+"_"+control_region+"_"+variable_name+"_rat0_"+current_sample+".root"  );
    	Analyzer->getCRHisto(presel_file, variable_bitcode|CR_bitcode|RAT1 , path_sim+s_CR+"_"+control_region+"_"+variable_name+"_rat1_"+current_sample+".root"  );
    	Analyzer->getCRHisto(presel_file, variable_bitcode|CR_bitcode|RAT2 , path_sim+s_CR+"_"+control_region+"_"+variable_name+"_rat2_"+current_sample+".root"  );
    }
  }
...
void CRHisto(int doCalc, int nCR, int nQU) {
  ...
  if( doNJetBinning ) {
    ...
    sjet.push_back("_rat0");
    sjet.push_back("_rat1");
    sjet.push_back("_rat2");
    }
  }
  ...
  #if !defined(Plot_PTRAT0) && !defined(Plot_PTRAT1) && !defined(Plot_PTRAT2)
...
//  for (int ij=0; ij<=2*(doNJetBinning); ij++){ //loop over inclusive, 0jet, 1jet //Check if Njet binning is implemented
    for (int ij=0; ij<=3*(useRatioBinning); ij++){ // loop over inclusive, pTratio 0, 1, 2
  ...
  for (int ic=0; ic<nCR; ic++){ //loop over CRs
    for (int iv=0; iv<nVARused; iv++){ //loop over mt, mvis, pt, muiso
      for (int iq=0; iq<nQU; iq++){ //loop over loose/tight
        ...
        for (int is=0; is<nSA; is++){
		cr.push_back(path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_"+ssa[is]+".root"); 
	}
		if(DEBUG) {
			 cout <<"CR Hist name "<<path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_data.root" << " " << squ[iq] << " " <<  scr[ic] << " " << m_path_img+s_CR+"_"+scr[ic]+"_"+squ[iq]+"_"+tvarCR[iv] << " " << tvarCR_l[iv]  << endl;
			  }
	...
	#ifdef Plot_PTRAT0
	std::vector<TString> cr_rat0; 
	for (int is=0; is<nSA; is++){
		cr_rat0.push_back(path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_rat0_"+ssa[is]+".root");
	}
	Analyzer->plotCR(cr_rat0, ty, path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_rat0_data.root", squ[iq], scr[ic]    , m_path_img+s_CR+"_"+scr[ic]+"_"+squ[iq]+"_"+tvarCR[iv]+"_rat0", tvarCR_l[iv] );
	cr_rat0.clear();

	#elif defined(Plot_PTRAT1)
	std::vector<TString> cr_rat1;
	for (int is=0; is<nSA; is++){
		cr_rat1.push_back(path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_rat1_"+ssa[is]+".root");
	}

	Analyzer->plotCR(cr_rat1, ty, path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_rat1_data.root", squ[iq], scr[ic]    , m_path_img+s_CR+"_"+scr[ic]+"_"+squ[iq]+"_"+tvarCR[iv]+"_rat1", tvarCR_l[iv] );
	cr_rat1.clear();

	#elif defined(Plot_PTRAT2)
	std::vector<TString> cr_rat2;
	for (int is=0; is<nSA; is++){
	        cr_rat2.push_back(path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_rat2_"+ssa[is]+".root");
	}

	Analyzer->plotCR(cr_rat2, ty, path_sim+s_CR+"_"+scr[ic]+"_"+tvarCR[iv]+"_rat2_data.root", squ[iq], scr[ic]    , m_path_img+s_CR+"_"+scr[ic]+"_"+squ[iq]+"_"+tvarCR[iv]+"_rat2", tvarCR_l[iv] );
	cr_rat2.clear();
	#endif
