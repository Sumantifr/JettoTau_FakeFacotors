...
void Preselection(TString argument) {
  ...
  if (argument.Contains("DY") || argument.Contains("all")) {
	  if(EMB == 1){
		myList.push_back(SelectionClass(DYfile, preselection_DY_J_EMB, _DY|_JTAU));                                                                                                                                 myList.push_back(SelectionClass(DYfile, preselection_DY_L_EMB, _DY|_LTAU));
		...
		myList.push_back(SelectionClass(DYfile, preselection_DY_EMB, 0));
    		...
    		myList.push_back(SelectionClass(DYfile, preselection_DY_J, _DY|_JTAU));
    		myList.push_back(SelectionClass(DYfile, preselection_DY_L, _DY|_LTAU));
    		myList.push_back(SelectionClass(DYfile, preselection_DY_TT, _DY|_TTAU));
    
    		myList.push_back(SelectionClass(DYfile, preselection_DY, 0));
	  }
  }
  if (myList.size()==0) {
