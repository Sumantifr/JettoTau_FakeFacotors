...
void fitFakeFactors(){
  ...
  vector<Int_t> modes; 
  modes.push_back(_QCD); modes.push_back(_QCD|_AI);
   if(CHAN!=kTAU){
     modes.push_back(_W_JETS); 
     modes.push_back(_W_JETS); 
     modes.push_back(_TT);
     }
...
// }
          ...
          ChannelJetcat.SetTextSize(0.03);
          ...
          TString jetpt20eta2p4Mode="incl";
	  if(jetpt20eta2p4_bins.at(imode) > 1) { { if(ijetpt20eta2p4==0) jetpt20eta2p4Mode="0jet"; else if(ijetpt20eta2p4==1) jetpt20eta2p4Mode="#geq1jet";;} }
	  if(jetpt20eta2p4_bins.at(imode) == 1) { jetpt20eta2p4Mode="incl"; } 
	  if(modes.at(imode) & _TT ) {
            if(ijetpt20eta2p4==0) {
