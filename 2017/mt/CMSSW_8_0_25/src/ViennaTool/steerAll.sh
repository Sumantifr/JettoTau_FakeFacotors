...
year=$(grep 'ver' "Settings.h" | awk -F'[=;]' '{print $2}' | tr -d '"')
...
python plotCorrections.py --channel $channel  --doNjetBinning $njetbinning --usePt_nonclosure_Wjets $usePt_nonclosure --embedding $embedding --year $year

./convert_inputs

if [ $embedding == 1 ]
then
	rsync -vhrP ViennaTool/fakefactor/data_${chan}/FF* ${output}/${chan}/EMB/.
	rsync -vhrP ViennaTool/sim/${chan}/* ${output}/${chan}/EMB/sim/.
	...
	rsync -vhrP ViennaTool/Images_EMB/data_${chan}/CR_* ${output}/${chan}/Images_EMB/.
elif [ $embedding == 0 ]
then 
	rsync -vhrP ViennaTool/fakefactor/data_${chan}/FF* ${output}/${chan}/NonEMB/.
	rsync -vhrP ViennaTool/sim/${chan}/* ${output}/${chan}/NonEMB/sim/.
	rsync -vhrP ViennaTool/Images_NonEMB/data_${chan}/ff_* ${output}/${chan}/Images_NonEMB/.
	rsync -vhrP ViennaTool/Images_NonEMB/data_${chan}/corr_* ${output}/${chan}/Images_NonEMB/.
	rsync -vhrP ViennaTool/Images_NonEMB/data_${chan}/CR_* ${output}/${chan}/Images_NonEMB/.
else
...
if [ $saveWork == 1 ] ; then
	...
	if [ -d /mnt/hephy/cms/suman.chatterjee/FakeFactor_MSSM/$analysis/save_$analysis ]; then
		rm -rf /mnt/hephy/cms/suman.chatterjee/FakeFactor_MSSM/$analysis/save_$analysis
	fi
