...
TString GlobalClass::getSRCutString(const Int_t mode, const Int_t categoryMode){
  ...
  TString s_genmatch, s_jetmode, s_ratmode, s_mode = "";
  ...
  if(useRatioBinning == 1){
  	if(mode & RAT0) s_ratmode = " * (alltaujet_pt[0]*1./alltau_pt[0]  < 1.25) ";
  	else if(mode & RAT1) s_ratmode = " * (alltaujet_pt[0]*1./alltau_pt[0] >= 1.25) * (alltaujet_pt[0]*1./alltau_pt[0] < 1.5) ";
  	else if(mode & RAT2) s_ratmode = " * (alltaujet_pt[0]*1./alltau_pt[0] >= 1.5) ";
  }
  ...
  return s_mode + s_genmatch + s_jetmode + s_ratmode; 
}
...
{  
  ...
  Int_t i_p,i_t,i_e,i_m,i_j,i_dR,i_jpt20eta2p4,i_pTratio;
  Int_t n_p=-99,n_e=-99,n_t=-99,n_j=-99,n_dR=-99,n_jpt20eta2p4=-99,n_pTratio=-99;
  ...
  i_pTratio=getpTratioIndex(mode,ind);
  


  if (mode & _W_JETS) {n_p=n_p_Wjets;n_e=n_e_Wjets;n_t=n_t_Wjets;n_j=n_j_Wjets;n_dR=n_dR_Wjets;n_jpt20eta2p4=n_jpt20eta2p4_Wjets;n_pTratio=n_pTratio_Wjets;}
  else if ( (mode & _DY) ) {n_p=n_p_DY;n_e=n_e_DY;n_t=n_t_DY;n_j=n_j_DY;n_dR=n_dR_DY;n_jpt20eta2p4=n_jpt20eta2p4_DY;n_pTratio=n_pTratio_DY;}
  else if ( (mode & _TT) && (mode & SR) )  {n_p=n_p_TT_SR;n_e=n_e_TT;n_t=n_t_TT;n_j=n_j_TT_SR;n_dR=n_dR_TT_SR;n_jpt20eta2p4=n_jpt20eta2p4_TT_SR;n_pTratio=n_pTratio_TT_SR;}
  else if ( (mode & _TT) ) {n_p=n_p_TT_CR;n_e=n_e_TT;n_t=n_t_TT;n_j=n_j_TT_CR;n_dR=n_dR_TT_CR;n_jpt20eta2p4=n_jpt20eta2p4_TT_CR;n_pTratio=n_pTratio_TT_CR;}
  //else if ( (mode & _TT) ) {n_p=n_p_TT;n_e=n_e_TT;n_t=n_t_TT;n_j=n_j_TT;}
  else if ( (mode & _QCD) && (mode & _AI) ) {n_p=n_p_QCD_AI;n_e=n_e_QCD;n_t=n_t_QCD;n_j=n_j_QCD;n_dR=n_dR_QCD;n_jpt20eta2p4=n_jpt20eta2p4_QCD;n_pTratio=n_pTratio_QCD;}
  else if ( (mode & _QCD) )     {n_p=n_p_QCD;n_e=n_e_QCD;n_t=n_t_QCD;n_j=n_j_QCD;n_dR=n_dR_QCD;n_jpt20eta2p4=n_jpt20eta2p4_QCD;n_pTratio=n_pTratio_QCD;}
  else {cout<<"getBin: Define a valid MODE!"<<std::endl;n_p=-99;n_e=-99;n_t=-99;n_dR=-99;}
  Int_t bin=i_e + i_p*n_e + i_t*n_p*n_e + i_j*n_t*n_p*n_e + i_dR*n_j*n_t*n_p*n_e + i_jpt20eta2p4*n_dR*n_j*n_t*n_p*n_e + i_pTratio*n_jpt20eta2p4*n_dR*n_j*n_t*n_p*n_e;
...
Int_t GlobalClass::nBins(const Int_t mode)
{
  ...
  */
  ...
  if (mode & _W_JETS) return(n_e_Wjets*n_p_Wjets*n_t_Wjets*n_m_Wjets*n_j_Wjets*n_dR_Wjets*n_jpt20eta2p4_Wjets*n_pTratio_Wjets);
  else if ( (mode & _DY) ) return(n_e_DY*n_p_DY*n_t_DY*n_m_DY*n_j_DY*n_dR_DY*n_jpt20eta2p4_DY*n_pTratio_DY);
  else if ( (mode & _TT) && (mode & SR) ) return(n_e_TT*n_p_TT_SR*n_t_TT*n_m_TT*n_j_TT_SR*n_dR_TT_SR*n_jpt20eta2p4_TT_SR*n_pTratio_TT_SR);
  else if ( (mode & _TT) ) {
    // if (DEBUG) {
    ...
    // }
    return(n_e_TT*n_p_TT_CR*n_t_TT*n_m_TT*n_j_TT_CR*n_dR_TT_CR*n_jpt20eta2p4_TT_CR*n_pTratio_TT_CR);
  }
  
  else if ( (mode & _QCD) && (mode & _AI) ) return(n_e_QCD*n_p_QCD_AI*n_t_QCD*n_m_QCD*n_m_QCD*n_j_QCD*n_dR_QCD*n_jpt20eta2p4_QCD*n_pTratio_QCD);
  else if ( (mode & _QCD) )   return(n_e_QCD*n_p_QCD*n_t_QCD*n_m_QCD*n_m_QCD*n_j_QCD*n_dR_QCD*n_jpt20eta2p4_QCD*n_pTratio_QCD);
...
Int_t GlobalClass::getpTratioIndex(const Int_t mode, const Int_t ind)
{
  // Binning in pt
  Int_t i_j=0;
  Double_t pTratio = event_s->alltaujet_pt->at(ind) / event_s->alltau_pt->at(ind); // here comes the important part
  
  // std::cout << "pTratio of the event: " << pTratio << std::endl;

  if (mode & _W_JETS) {
    for(Int_t i=0;i<n_pTratio_Wjets;i++) if (pTratio>=npTratio_cuts_Wjets[i]) i_j++;
    return(--i_j);
  } else if ( (mode & _DY) ) {
    for(Int_t i=0;i<n_pTratio_DY;i++) if (pTratio>=npTratio_cuts_DY[i]) i_j++;
    return(--i_j);
  } else if ( (mode & _TT) && (mode & SR) ) {
    for(Int_t i=0;i<n_pTratio_TT_SR;i++) if (pTratio>=npTratio_cuts_TT_SR[i]) i_j++;
    return(--i_j);
  } else if ( (mode & _TT) ) {
    for(Int_t i=0;i<n_pTratio_TT_CR;i++) if (pTratio>=npTratio_cuts_TT_CR[i]) i_j++;
    return(--i_j);
  } else if ( (mode & _QCD) ) {
    for(Int_t i=0;i<n_pTratio_QCD;i++) if (pTratio>=npTratio_cuts_QCD[i]) i_j++;
    return(--i_j);
  } else {cout<<"Define a valid mode!"<<std::endl;return(-99);}
}
...
Int_t GlobalClass::getpTratio(const Int_t mode, const Int_t ind)
{
  if(mode & _W_JETS) return N_pTratio_Wjets;
  else if(mode & _QCD) return N_pTratio_QCD;
  else if(mode & _DY) return N_pTratio_DY;
  else if(mode & _TT && mode & SR) return N_pTratio_TT_SR;
  else if(mode & _TT) return N_pTratio_TT_CR;
  else {cout<<"Define a valid mode!"<<std::endl;return(-99);}
}
